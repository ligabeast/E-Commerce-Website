...
@section('content')
    <form  id="app" class="p-10 flex flex-col wrap space-y-5 border-t-2 border-black my-5">
        {{ csrf_field() }}
        ...
        <div class="flex flex-col space-y-2">
            ...
            <div class="flex flex-row">
                <input class="w-8" required type="radio" v-model="Zustand"  id="Neu" value="Neu" name="state">
                <label for="Neu">Neu</label>
            </div>
            <div class="flex flex-row">
                <input class="w-8" required type="radio" v-model="Zustand"  id="Gebraucht" value="Gebraucht" name="state">
                <label for="Gebraucht">Gebraucht</label>
            </div>
        </div>
        <div class="flex flex-col space-y-6 py-5">
            <div class="relative">
                <input v-model="Titel" required name="title" id="Angebotstitel" class="placeholder-transparent peer border border-black text-lg rounded-xl px-5 py-3 w-full outline-0" placeholder="Titel">
        ...
        <div class="flex flex-col space-y-3">
            ...
            <input v-model="Kategorie" name="category" readonly class="bg-gray-200 border border-black text-lg rounded-xl px-5 py-3 w-full outline-0" value="{{$category}}">
        </div>
        <div class="flex flex-col space-y-3">
            <p class="font-bold  text-lg">Beschreibung (max. 1000Zeichen)</p>
            <textarea v-model="Beschreibung" required maxlength="1000" name="description" class="w-full text-lg h-64 outline-0 border-2 border-gray-400 p-5 rounded-xl resize-none"></textarea>
        </div>
        <div class="flex justify-between ">
            <div class="flex items-center space-x-5">
                <p class="font-bold text-lg">Preis:</p>
                <input v-model="Preis" required name="price" id="price" type="number" step=".01" class="bg-gray-200 border border-black text-lg rounded-xl px-5 py-3 w-32 outline-0">
            </div>
            <button @click.prevent="myMethod" id="submit" type="submit" class="relative inline-flex items-center px-12 py-3 overflow-hidden text-lg font-medium text-blue-600 border-2 border-blue-600 rounded-full hover:text-white group hover:bg-gray-50">
    ...
    <script type="module" src="{{asset('scripts/listItem_vue.js')}}"></script>
@endsection
